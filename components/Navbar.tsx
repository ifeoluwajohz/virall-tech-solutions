    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 ">
        <div className="flex justify-between items-center h-15 ">
          <Link
            className="font-medium flex text-xs justify-center items-center gap-x-2 text-md hover:underline transition-all"
            href="/"
          >
            <Fingerprint />
            Lienxify
          ...
          <div className="hidden md:flex space-x-6">
            {NAV_LINKS.map(({ label, href }) => (
              <Link key={label} href={href} className="duration-200 text-xs">
                {label}
              </Link>
            ))}
          </div>
      ...
      >
        ...
        <div className="h-2 bg-transparent mt-2"></div>
      </div>
    </nav>
  );
};
